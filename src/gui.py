...
class App(WhatsAppMixin, HealthMixin):
    ...
    def _handle_msg(self, kind: str, payload: object) -> None:
        ...
        # ------ Counter Activity messages ------
            ...
            if info.get("agreement", 1.0) < 0.6:
                # a folder spanning several months is a real thing to know
                self._fsh_log(
                    f"    only {info.get('agreement', 0):.0%} of the dates fall "
                    f"in {info.get('month', 0):02d}/{info.get('year', 0)} — "
                    f"these files span more than one month")
    ...
    def _fsh_log(self, msg: str) -> None:
        """One status line. The tab already has a summary pane, so a second
        log box would only compete with it."""
        try:
            self.fsh_progress_label.configure(text=msg)
    ...
    def _fsh_run(self) -> None:
        if self._fsh_worker is not None and self._fsh_worker.is_alive():
            return
        folder = (self.zenith_fh_input_dir.get() or "").strip()
        if not folder or not Path(folder).is_dir():
            messagebox.showerror(
                "Flight schedule history",
                "Pick the logs folder in 'Folders' above first — the same "
                "ModificationHistory files the history audit reads.")
            return
        ...
        self._fsh_worker = threading.Thread(
            ...
            args=(Path(folder),
                  Path(self.zenith_fh_output_dir.get().strip()
                       or str(Path.home())),
                  self._zenith_session if self.fsh_with_roster.get() else None),
            daemon=True)
        self._fsh_worker.start()

    def _fsh_worker_run(self, folder, out_dir, session) -> None:
        ...
        try:
            ...
            # The month is voted from the dates in the files. Asking for one
            # is asking to be given the wrong one: a run that defaulted to
            # today's month once relabelled a whole month of another.
            month, year, _votes, agree = fsh.detect_period(events)
            if month is None:
                raise ValueError(
                    "These files carry no readable flight date, so the period "
                    "cannot be established.")
            self._post(MSG_FSH_LOG,
                       f"Period read from the files: {month:02d}/{year} "
                       f"({agree:.0%} agreement)")
    ...
    def _build_zenith_history_tab(self, parent: ttk.Frame) -> None:
        ...
        # ----- Flight schedule history -----
        # Same folder, same files, a different question: the audit above reads
        # them per PNR to judge a reissue; this reads them per FLIGHT to count
        # how often the schedule itself moved. No month is asked for -- it is
        # voted from the dates in the files, because a run that defaults to
        # today's month relabels somebody else's month.
        sched_body = self._section(
            parent, "Flight schedule history",
            help_text=(
                "How many times each flight's date or time changed, how often "
                "it was swapped or cancelled, by how much and how late — per "
                "flight and per route. Uses the folder above; the period is "
                "read from the files."
            ),
        )
        sched_row = ttk.Frame(sched_body)
        sched_row.pack(fill="x", padx=2, pady=(0, 2))
        self.btn_fsh_run = ttk.Button(
            sched_row, text="Build schedule history", command=self._fsh_run)
        self.btn_fsh_run.pack(side="left")
        self.btn_fsh_stop = ttk.Button(
            sched_row, text="Stop", command=self._fsh_stop, state="disabled",
            style="Danger.TButton")
        self.btn_fsh_stop.pack(side="left", padx=(8, 0))
        ttk.Checkbutton(
            sched_row, variable=self.fsh_with_roster,
            text="also fetch the roster (scheduled times, aircraft, and the "
                 "share of flights changed) — needs sign-in",
        ).pack(side="left", padx=(12, 0))
        self.fsh_progress_label = ttk.Label(sched_body, text="",
                                            style="Hint.TLabel")
        self.fsh_progress_label.pack(anchor="w", padx=2)
