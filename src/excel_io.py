...
def write_zenith_pnr_misuse_audit(path: Path, report) -> None:
    ...
    cover.append(["PNR-fallback groups (ticket# unknown)", getattr(report, "fallback_groups", 0)])
    cover.append(["  ticket-lifecycle flags (refund-of-flown / self-refund / downgrade /"])
    cover.append(["  churn) run ONLY on real-ticket groups; refund-of-flown needs Flown"])
    cover.append(["  events to appear in the log (count above) or it stays inactive."])
    cover.append([])
